 ...
 * 
 * @author Damien Di Fede, Anderson Mills, Chainz
 * 
 */
public class ChainzOscil extends UGen {
	/**
	 ...
	 *            the waveform we will oscillate over
	 */
	public ChainzOscil( float frequencyInHertz, float amplitude, Waveform waveform ) {
	 ...
	 *            the amplitude
	 */
	public ChainzOscil( float frequencyInHertz, float amplitude ) {
	...
	public ChainzOscil( Frequency frequency, float amplitude ) {
	...
	public ChainzOscil( Frequency freq, float amp, Waveform wave ) {
		super();
	...
	/**
	 * Sets the frequency of this Oscil. You might want to do this to change the frequency of this Oscil in response to
	 * a button press or something. For controlling frequency continuously over time you will usually want to use the
	 * frequency input.
	 * 
	 * @param hz
	...
	/**
	 * Set the amount that the phase will be offset by. Oscil steps its time from 0 to 1, which means that the phase is
	 * also normalized. However, it still makes sense to set the phase to greater than 1 or even to a negative number.
	 * 
	 * @param newPhase
	 */
	...
	/**
	 * Resets the time-step used by the oscillator to be equal to the current phase value. You will typically use this
	 * when starting a new note with an Oscil that you have already used so that the waveform will begin sounding at the
	 * beginning of its period, which will typically be a zero-crossing. In other words, use this to prevent clicks when
	 * starting Oscils that have been used before.
	 */
	public void reset() {
		step = fPhase;
	}
